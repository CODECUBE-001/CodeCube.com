const phpFuncs = siteDir + '/core/php/amvc.php',
    reqURI = window.location.pathname;

var lastPage = curPage,
    aLoadHistory = [curPage],
    routes,
    temp,
    xck,
    loadtime,
    ltimecounter,
    AMVCdata = { init: 'home' },
    pUri = window.location.href.split('/');
...
function genHTML(pattern) {
    ...
    for (let cn of pattern) {
        ...
        let c_patn = pattern[cn]; // here's some kinda mistake.. should've been cn istead of pattern[cn]
...
const elIndex = (el = '') => {
    ...
    var pn = document.querySelector(el).parentNode,
        n_xs = Array.prototype.slice.call(pn.children),
